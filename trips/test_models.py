...
class TestModels(TestCase):
    ...
    def test_review_default(self):
        ...
        review = Review.objects.create(trip=trip, user=user, comment='This is a comment')
        self.assertEqual(review.rating, 3)

    def test_trip_str(self):
        trip = Trip.objects.get(name='Test Trip')
        self.assertEqual(trip.__str__(), 'Test trip at Test destination')
    ...
    def test_review_str(self):
        ...
        trip.registered_users.add(user)
        review = Review.objects.create(trip=trip, user=user, comment='This is a comment')
        self.assertEqual(review.__str__(), f'User {user.username} commented {review.comment} with a rating of {review.rating}')
    ...
    def test_Request_str(self):
        ...
        self.assertEqual(request.__str__(), f'User {user.username} requested the destination {request.destination} with a description of {request.description}' )
